---
title: "E23a - R project: Image classifier using Eigenfaces"
author: Lecaro, Gabriela; Pinto, Carlos Eduardo
output: pdf_document
---

```{r set-up, eval=FALSE, echo=FALSE, warning=FALSE}
#setwd("C:/Users/IPAUser/Desktop/R Project/") # Please change it to your working directory first, before running the 'here' package
...
Disclaimer: Photos were found on Google images. We do not hold intellectual property rights over this material, and are only using them for non-profit, academic purposes.

In order to construct the dataset, first download the necessary image files contained in appropriately named folders as outlined in the read me file. Once imported, each large list object will correspond to a specific subject/celebrity.  

These objects are then combined, stacked and split into training and testing datasets. The training dataset is the primary dataset, used for replication and extension parts. The testing dataset will only be used in the extension portion.
...
###############################################
## Splitting into training, testing and rotating
...
Here is a sample of 12 photos in the training dataset

```{r test sample, echo=FALSE, warning=FALSE}
  
  par(mfrow=c(3,4)) # To contain all images in single frame
  for(i in 1:12){
    plot(test_pool[[i]])
  }
  par(mfrow=c(1,1)) # Reset back to default mode
...
#### Part 2: Average faces and eigenfaces
Here we are calculating and plotting the mean/average faces.

```{r average faces and eigenfaces, echo=FALSE, warning=FALSE}  
#########################################
## Part II: Average faces and eigenfaces
...
```{r PCA, echo=FALSE, warning=FALSE}  
  ...
  # Note that this line takes substantial time for running
...
```{r Projecting and reconstructing from eigenfaces, echo=FALSE, warning=FALSE} 
#################################
## Part IV: Plotting, projecting and reconstructing faces
...
#### Part V: Classifying and matching photos

```{r Classifying and matching, echo=FALSE, warning=FALSE} 
##########################################
...
### EXTENSION: IMAGE CLASSIFIER AND PREDICTOR USING KERAS AND TENSORFLOW
#### Part VI: Splitting dataset and encoding

```{r Splitting dataset and encoding, echo=FALSE, warning=FALSE} 
##########################################################################################################
...
#### Part VII: Building the model

```{r Building the model, echo=FALSE, warning=FALSE} 
  # Model Building
  ...
  print(summary(model.card))

```

#### Part VIII: Model and results

```{r Fitting model and results, eval=FALSE, echo=FALSE, warning=FALSE} 
  #fit model
